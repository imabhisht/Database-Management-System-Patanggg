...
function start_me(value1){
    eel.list_order(value1)(function(ret){
        ...
        loadtableData(personData,value1);
    })   
}
 
function loadtableData(personData,value1){
    if(value1 === 'Shipped'){
        var echo1 = 'OutForDelivery'
        var echo2 = 'Out For Delivery'
    }
    else if(value1 === 'OutForDelivery'){
        var echo1 = 'Delivered'
        var echo2 = 'Successfully Delivered'
    }
    ...
    for(let person of personData){
        ...
        <button type="button" class="btn btn-warning" onclick="verify('${person.orderid}','${person.userid}','${echo1}')";>${echo2}</button>
        </td></tr>`;
        }
...
function status_change(product_id,user_id,change){
    if(change === 'OutForDelivery'){
    eel.status_change(product_id,user_id,change,'Shipped') /*(ORDER_ID,USER_ID,CHANGED_STATUS,CURRENT_STATUS)*/
    start_me('Shipped')}
    else if(change === 'Delivered'){
        eel.status_change(product_id,user_id,change,'OutForDelivery') /*(ORDER_ID,USER_ID,CHANGED_STATUS,CURRENT_STATUS)*/
        start_me('OutForDelivery')}
}
