...
function start_me(){
    eel.list_order('Approved')(function(ret){
...
function loadtableData(personData){
    ...
    for(let person of personData){
        ...
        <!--<button type="button" class="btn btn-warning" onclick="verify('${person.orderid}','${person.userid}','DelayApproval')";>Delay Approval</button>-->
        </td></tr>`;
        }
...
eel.orders_generate()(function(ret){
    if(ret == 'Sucess'){
...
function verify_forshipping(){
    const modelbody2 = document.getElementById('staticBackdrop')
    let dataHtml =`
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
        <div class="modal-body">
            <p>Ready all orders for Shipping ?<br><br><b>Note: YOU CAN'T UNDO THIS!!</b></p> 
        </div>
        <div class="modal-footer">
            <button type="button" class="btn btn-danger" data-dismiss="modal">No</button>
            <button type="button" class="btn btn-success" data-dismiss="modal" onclick="verify_forshipping2()";>Yes</button>
        </div>
        </div>
    </div>
    </div>;`
